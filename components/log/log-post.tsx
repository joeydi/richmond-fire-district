...
export function LogPost({ post, canEdit = false, canDelete = false, onDeleted, showPermalink = true }: LogPostProps) {
  ...
  return (
    <>
      <div className="flex flex-wrap flex-col xl:flex-row xl:items-start justify-between gap-4">
        <div className="xl:w-[25%] xl:m-5 shrink-0 text-sm text-slate-500">
          <span className="font-medium text-slate-700">{formattedDate}</span>
          <br />
          {authorName}
        </div>
        <Card className="xl:w-[50%] grow">
          <CardContent className="space-y-4">
            <LogPostContent content={post.content} />

            {post.images.length > 0 && <LogImageGallery images={post.images} canEdit={false} />}
          </CardContent>
        </Card>
        <div className="xl:w-[12%] xl:m-5 flex items-center justify-end gap-1">
          {showPermalink && (
